...
static void *thread(void *_) {
  ...
  for (;;) {
    ...
    if (err != LWMQTT_SUCCESS) {
      Log_Debug("failed lwmqtt_unix_network_select: %d\n", err);
      exit(1);
    }
    ...
    if (available) {
      ...
      if (err != LWMQTT_SUCCESS) {
        Log_Debug("failed lwmqtt_unix_network_peek: %d\n", err);
        exit(1);
      }
      ...
      if (available_bytes > 0) {
        ...
        if (err != LWMQTT_SUCCESS) {
          Log_Debug("failed lwmqtt_yield: %d\n", err);
          exit(1);
        }
      }
    }
    ...
    if (err != LWMQTT_SUCCESS) {
      Log_Debug("failed lwmqtt_keep_alive: %d\n", err);
      exit(1);
    }
...
int main() {
  ...
  if (err != LWMQTT_SUCCESS) {
    Log_Debug("failed lwmqtt_unix_network_connect: %d\n", err);
    exit(1);
  }
  ...
  if (err != LWMQTT_SUCCESS) {
    Log_Debug("failed lwmqtt_connect: %d (%d)\n", err, return_code);
    exit(1);
  }
  ...
  if (err != LWMQTT_SUCCESS) {
    Log_Debug("failed lwmqtt_subscribe: %d\n", err);
    exit(1);
  }
  ...
  if (r < 0) {
    Log_Debug("failed pthread_mutex_init: %d\n", r);
    exit(0);
  }
  ...
  if (r < 0) {
    Log_Debug("failed pthread_create: %d\n", r);
    exit(1);
  }
  ...
  for (;;) {
    ...
    if (err != LWMQTT_SUCCESS) {
      Log_Debug("failed lwmqtt_publish: %d\n", err);
      exit(1);
    }
